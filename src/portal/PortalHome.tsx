 ...
 *
 * Lists every service registered in `./useCases.ts`. Composition only —
 * no visual primitives invented here. If you need a new card shape,
 * open an issue on the design system.
 */
export function PortalHome() {
  ...
  return (
    <Workspace>
      ...
      <Section title="Quick links" count={3}>
        <QuickLinks>
          <QuickLink icon="document" label="Open occurrence list" onClick={() => undefined} />
          <QuickLink icon="user" label="My active assignments (3)" onClick={() => undefined} />
          <QuickLink icon="key" label="Configure inference resources" onClick={() => navigate("/resources")} />
        </QuickLinks>
      </Section>
    </Workspace>
  );
